...
export default function FindAssociatePage() {
  const [searchQuery, setSearchQuery] = useState("")
  const [verifiedOnly, setVerifiedOnly] = useState(false)

  const { data, isLoading } = useSWR<{ success: boolean; associates: Associate[] }>("/api/associates", fetcher)
  ...
  const filteredAssociates = associates.filter((associate) => {
    ...
    const matchesVerified = !verifiedOnly || associate.verified
    ...
    <main className="min-h-screen bg-gradient-to-b from-green-50 to-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-[#125007] mb-4 text-balance">Find Expert Land Associates</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto text-pretty">
            Connect with verified land associates and consultants across India. Get expert guidance for your land investment.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 md:p-8 mb-12">
          ...
          {!isLoading && (
            <div className="text-sm text-gray-600 mt-4">
              Found {filteredAssociates.length} {filteredAssociates.length === 1 ? "associate" : "associates"}
            </div>
          )}
        </div>
        ...
        {/* Associates Grid */}
        {!isLoading && filteredAssociates.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {filteredAssociates.map((associate) => {
              const joined = formatJoined(associate.joined)
              return (
                <div
                  key={associate.id}
                ...
                >
                  <div className="bg-gradient-to-r from-[#125007] to-[#4a7c2e] p-6 text-white">
                    ...
                    <h3 className="text-xl font-bold mb-1 capitalize">{associate.name}</h3>
                    <p className="text-green-100 text-sm">Land Associate</p>
                  </div>
        ...
        {!isLoading && filteredAssociates.length === 0 && (
          <div className="text-center py-16 mb-12">
            <p className="text-gray-600 text-lg mb-4">
              {associates.length === 0 ? "No associates have registered yet." : "No associates found matching your search."}
            </p>
            {associates.length > 0 && (
              <button
                onClick={() => {
        ...
        <div className="bg-gradient-to-r from-[#125007] to-[#4a7c2e] rounded-lg p-8 text-white mb-12">
          <h2 className="text-2xl font-bold mb-8">Why Choose Land2Land Associates?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-green-100 text-sm">All associates are verified professionals with proven track records</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-3">
