...
export default function PropertyFormStep1({ formData, onChange }: any) {
  ...
  // Load the logged-in user so associates see their own name as the Seller / Associate
  useEffect(() => {
    const loadCurrentUser = async () => {
      try {
        ...
        if (data?.user) {
          ...
          // For associates creating a new listing, default the seller/associate to themselves
