      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {locationCategories.map((category) => (
            <div
              key={category.title}
              className="bg-white hover:bg-gradient-to-br hover:from-white hover:to-[#2d5016]/2 transition-all duration-300 rounded-lg overflow-hidden border border-gray-100 hover:border-[#2d5016]/20"
            >
              ...
              <div className="relative h-40 md:h-48 overflow-hidden bg-gray-100">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-[#2d5016]/20 to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300" />
              ...
              <div className="p-3 md:p-4 space-y-2">
                {category.items.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="flex items-center justify-between p-2 md:p-2.5 rounded-lg text-gray-700 hover:bg-[#2d5016]/10 hover:text-[#2d5016] transition-all duration-200 border border-transparent hover:border-[#2d5016]/20"
                  >
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <div className="text-[#2d5016] flex-shrink-0 hover:scale-125 transition-transform duration-200">
                        {item.icon}
                      </div>
                      <span className="text-xs md:text-sm font-medium truncate">{item.name}</span>
                    </div>
                    <ArrowRight className="w-3 h-3 md:w-3.5 md:h-3.5 opacity-0 hover:opacity-100 transition-all duration-200 ml-2 flex-shrink-0 text-[#2d5016]" />
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="p-4 md:p-5 space-y-2.5">
                {category.items.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="flex items-center justify-between p-2.5 md:p-3 rounded-lg text-gray-700 hover:bg-[#2d5016]/10 hover:text-[#2d5016] transition-all duration-200 border border-transparent hover:border-[#2d5016]/20"
                  >
                    ...
                    <ArrowRight className="w-3.5 h-3.5 md:w-4 md:h-4 opacity-0 hover:opacity-100 transition-all duration-200 ml-2 flex-shrink-0 text-[#2d5016]" />
                  </Link>
                ))}
              </div>
            </div>
